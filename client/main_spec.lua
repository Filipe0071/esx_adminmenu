	...
	update = function(self, players)
		if not players or type(players) == "boolean" then
			ESX.TriggerServerCallback("requestServerPlayers", function(players)
				...
				SendNUIMessage({players = players})
			end)
			return
		end
		SendNUIMessage({players = players})
...
CreateThread(function()
	while true do
		if Panel.selectedPlayer then
			...
			SendNUIMessage({playerInfo = {"GodMode: " .. (GetPlayerInvincible(pid) and RED .. "Enabled" or GREEN .. "Disabled") .. "</span>", "AntiRagdoll: " .. (not CanPedRagdoll(targetPed) and RED .. "Enabled" or GREEN .. "Disabled") .. "</span>", "Health: " .. GetEntityHealth(targetPed) .. "/" .. GetEntityMaxHealth(targetPed), "Armor: " .. GetPedArmour(targetPed) .. "/" .. GetPlayerMaxArmour(pid)}})
		else
			SendNUIMessage({playerInfo = false})
		end
		Wait(1000)
	end
end)
